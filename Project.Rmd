...
HP <- HP %>%
  mutate(GarageType = as.character(GarageType),
         GarageType = replace_na(GarageType, "No Bsmt"))
HP$GarageType = factor(HP$GarageType, levels = c("2Types", "Attchd", "Basment", "BuiltIn",	"CarPort",	"Detchd", "No Bsmt"))

###' GarageFinish
HP <- HP %>%
  mutate(GarageFinish = as.character(GarageFinish),
         GarageFinish = replace_na(GarageFinish, "No Bsmt"))
HP$GarageFinish = factor(HP$GarageFinish, levels = c("Fin", "RFn", "Unf", "No Garage"))

###' GarageQual
HP <- HP %>%
  mutate(GarageQual = as.character(GarageQual),
         GarageQual = replace_na(GarageQual, "No Bsmt"))
HP$GarageQual = factor(HP$GarageQual, levels = c("Ex", "Gd", "TA", "Fa",	"Po",	"No Bsmt"))
