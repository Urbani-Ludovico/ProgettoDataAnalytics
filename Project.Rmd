...
```{r, message = F}
HP_Excluded = HP
#HP$FrontageExists <- factor(!is.na(LotFrontage), levels = c(F, T), labels = c("no", "yes")) # Questo non ha senso
###' LotShape
recode_map <- c(
  "Reg" = "Regular",
  "IR1" = "Irregular",
...
HP_Excluded$FenceExists = factor(HP_Excluded$FenceExists, levels = c("F", "T"))
#HP$OpenPorchExists <- factor(OpenPorchSF != 0, levels = c(F, T), labels = c("no", "yes")) # Questo non ha senso
#HP$EnclosedPorchExists <- factor(EnclosedPorch != 0, levels = c(F, T), labels = c("no", "yes")) # Questo non ha senso
#HP$ScreenPorchExists <- factor(ScreenPorch != 0, levels = c(F, T), labels = c("no", "yes")) # Questo non ha senso
