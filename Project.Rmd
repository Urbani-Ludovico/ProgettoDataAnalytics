...
Con un p-value di `r summary(mod)[[4]][2,4]` si nota che più le case sono recenti, più il prezzo tende a crescere. Ma le case costruite prima del 1910/1920, quindi le case ritenute storiche, continuano a manterere un costo di vendita abbastanza alto.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si nota che più la superificie rivestita di muratura è ampia, più il prezzp delle case tende a crescere. Questo in quanto il rivestimento in muratura tende a preservare meglio le condizioni delle pareti nel tempo.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si nota che le case con delle fondamenta in calcestruzzo gettato in opera, tendono ad avere un prezzo più alto.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si nota che le case con un ampio seminterrato tendono ad avere un prezzo più alto rispetto alle altre.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si nota che più la qualità del riscaldamento è alta e più il prezzo della casa di alza. Inoltre la maggioranza delle case con il prezzo più alto rientrano nella fascia migliore di riscaldamento.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si nota che più la metratura del piano terra è elevata, più il prezzo della casa tende a salire.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si nota che più l'area del Piano terra aumenta, più il prezzo tende ad aumentare. È possibile che questo sia dovuto al fatto che le persone associano a piu bagni un prezzo superiore. Si nota oltre che piu l'area aumenta, piu l'incertezza su quale prezzo attribuire alla casa.
...
Con un p-value di `r summary(mod)[[1]]$"Pr(>F)"[1]` si deduce che più bagni si ha, più il prezzo tende a aumentare. È possibile che questo sia dovuto al fatto che le persone associano a piu bagni un prezzo superiore.
...
Con un p-value di `r summary(mod)[[1]]$"Pr(>F)"[1]` si deduce che più Stanze si ha, più il prezzo tende a aumentare. È possibile che questo sia dovuto al fatto che le persone associano a Stanze un prezzo superiore. Si puo osservare degli outlier a 2 e 14  Stanze dovute alle mancanza dei dati (esiste un solo dato per ognuna).
...
Con un p-value di `r summary(mod)[[1]]$"Pr(>F)"[1]` si deduce che più Camini si ha, più il prezzo tende a aumentare. È possibile che questo sia dovuto al fatto che le persone associano a Camini un prezzo superiore.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si deduce che più nuovo il garage è, più il prezzo tende ad aumentare. È possibile che questo sia dovuto al fatto che le persone associano ai garage piu nuovi un prezzo superiore.
...
Con un p-value di `r summary(mod)[[1]]$"Pr(>F)"[1]` si deduce che più auto che possono stare nel garage, più il prezzo tende ad aumentare. È possibile che questo sia dovuto al fatto che le persone associano ai posti auto un prezzo superiore.
...
Con un p-value di `r summary(mod)[[4]][2,4]` si deduce che più grande è il garage, più il prezzo tende ad aumentare. È possibile che questo sia dovuto al fatto che le persone associano al area del garage un prezzo superiore. Si nota che con un garage di 0m^2, quindi nessun garage, il prezzo tende ad essere minore.
...
Con un p-value di `r summary(mod)[[1]]$"Pr(>F)"[1]` si deduce che più i numeri di bagni aumenta, più il prezzo tende a aumentare. È possibile che questo sia dovuto al fatto che le persone associano al numeri di bagni à un prezzo superiore.
...
Dal grafico sovrastante possiamo notare che più le date degli ultimi lavori efffettuti sulle case è recente più le abitazioni hanno una qualità complessiva migliore, inoltre anche il prezzo è maggiore.
Inoltre possiamo notare che da circa l'inizio del nuovo millennio c'è stato un notevole riguardo alla qualità complessiva dell'abitazione.
Inoltre possiamo notare delle fasce di prezzo che sono composte principalmente da un colore solo, il che ci può far dedurre che sia la qualità generale dell'abitazione sia la data degli ultimi lavori sulla casa incidono sul prezzo in modo significativo.
...
Da questo grafico, invece, notiamo che il prezzo dell'abitazione non ha fasce distinte di colore, ma si può evincere che c'è un netto miglioramento delle condizioni del riscaldamento da l'inizio del 1990 circa. Con una qualità del riscaldamento che è passata ad eccellente nella maggior parte delle case che hanno avuto lavori.
...
Questo grafico invece evidenzia rispetto all'anno di costruzione anche il tipo di fondamenta utilizzate per la costruzione della casa.
Anche in questo grafico, come il precedente, non abbiamo fasce, quindi il prezzo di vendita della casa non viene influenzato dalla tipologia di fondamenta utilizzate, ma si può evincere che le tipologia di fondamenta sono in relazione ad un periodo storico, nel senso che recentemente si utilizzano principalmente quelle in calcestruzzo colato, mentre prima del 1980 erano più di moda altre tecniche.

```{r, message = F, warning = F}
total_area = HP$X1stFlrSF + HP$GrLivArea + HP$MasVnrArea
ggplot(HP, aes(x = total_area, y = SalePrice, color = factor(TotRmsAbvGrd))) +
    geom_point() +
    labs(x = "area totale", 
         y = "prezzo di vendita", 
...
Dal grafico possiamo notare che all'aumentare dell'area totale c'è anche un aumento del numero delle camere, questo aumento è più distribuito, meno netto rispetto alle tecniche del precedente grafico. Con l'aumento dell'area totale della casa c'è anche un significativo aumento del prezzo di vendita.

```{r, message = F, warning = F}
ggplot(HP, aes(x = X1stFlrSF + GrLivArea, y = log(SalePrice), color = factor(FullBath))) +
    geom_point() +
...
Nel grafico sovrastante si può notare che all'aumentare della superficie del primo piano c'è anche un aumento del numero dei bagni completi all'interno della casa. C'è anche un vigoroso aumento di prezzo annesso, L'aumento vorticoso di prezzo in relazione alla superficie è plausibile considerando che non è la completa superficie della casa ma solamente una parte.
...
Dal grafico sovrastante si può avincere che, come con il riscaldamento, anche in questo caso la qualità dei caminetti è mediamente in leggero aumento, in modo meno significativo; mentre con il prezzo di vendita non ci sono delle fasce di prezzo differenti per qualià differenti dei caminetti, dunque possiamo dedurre che la qualità del caminetto non incide significativamente sul prezzo di vendita.
...
Nel grafico si può notare una come con le fondamenta, possiamo notare delle distinzioni per epoche, il garage soprattutto negli ultimi decenni è stato costruito in cocomitanza con gli ultimi lavori effettuati.