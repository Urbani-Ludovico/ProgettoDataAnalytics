...
HP$GarageType <- factor(
    HP$GarageType, 
    levels = c("2Types", "Attchd", "Basment", "BuiltIn", "CarPort",	"Detchd", "No Garage"))
...
```{r, warning = F}
vars <- c("SalePrice", "AlleyExists", "LotShapeReduced", "HouseAge", "MSZoningReduced", "GarageExists", "UtilitiesReduced", "OverallCondReduced", "FirePlaceExists", "BsmtExists", "FenceExists", "MiscFeatureExists", "TotalBath")
...
mod <- analysis.qualitative(
    SalePrice, ExterQual,
    second_var.lab = "qualità esterna"
)
```
...
ggplot(HP, aes(x = YearRemodAdd, y = log(SalePrice), color = factor(OverallQual))) +
    geom_point() +
    labs(x = "ultimi lavori (anno)", y = "prezzo di vendita", color = "Qualità complessiva") +
    theme_minimal()
```
...
ggplot(HP, aes(x = YearRemodAdd, y = log(SalePrice), color = factor(HeatingQC))) +
    geom_point() +
    labs(x = "ultimi lavori eseguiti", y = "prezzo di vendita", color = "Qualità del riscaldamento") +
    theme_minimal()
```
...
ggplot(HP, aes(x = YearBuilt, y = SalePrice, color = Foundation)) +
    geom_point() +
    labs(x = "anno di costruzione", y = "prezzo di vendita", color = "Fondamenta") +
    theme_minimal()
```
...
ggplot(HP, aes(x = total_area, y = SalePrice, color = factor(TotRmsAbvGrd))) +
    geom_point() +
    labs(x = "area totale", y = "prezzo di vendita", color = "Numero di stanze") +
    theme_minimal()
```
...
ggplot(HP, aes(x = X1stFlrSF + GrLivArea, y = log(SalePrice), color = factor(FullBath))) +
    geom_point() +
    labs(color = "Numero di bagni", x = "metratura piano terra", y = "prezzo di vendita") +
    theme_minimal()
```
...
ggplot(HP, aes(x = GarageYrBlt, y = SalePrice, color = GarageArea.cut)) +
    geom_point(na.rm = T) +
    labs(x = "anno di costruzione del garage", y = "prezzo di vendita", color = "Area abitabile") +
...
ggplot(HP, aes(x = YearRemodAdd, y = SalePrice, color = factor(FireplaceQu))) +
    geom_point() +
    labs(x = "ultimi lavori eseguiti (anno)", y = "prezzo di vendita", color = "Qualità estintori") +
    scale_color_discrete(na.translate = F) +
    theme_minimal()
```
...
year_gap <- cut(
    HP$YearRemodAdd, 
    breaks = c(1930, 1950, 1980, 2000, max(HP$YearRemodAdd)),
    labels = c("1930-1950", "1950-1980", "1980-2000", "2000-now"))

ggplot(HP, aes(x = GarageYrBlt, y = SalePrice, color = factor(year_gap))) +
    geom_point() +
    labs(x = "anno di costruzione del garage", y = "prezzo di vendita", color = "Ultimi lavori (anno)") +
    scale_color_discrete(na.translate = F) +
    theme_minimal()
```
