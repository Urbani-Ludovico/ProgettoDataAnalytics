...
for (package in c("ggplot2", "ggcorrplot", "ggpubr", "dplyr", 
...
HP <- HP %>%
  mutate(BsmtFinType2 = as.character(BsmtFinType2),
         BsmtFinType2 = replace_na(BsmtFinType2, "F"))
...
Heapmap.vars <- c(
    ...
    "YearRemodAdd", "Exterior1st", "ExterQual", "ExterCond", "Foundation", "BsmtQual", 
    "BsmtCond", "BsmtExposure", "BsmtFinSF1", "BsmtFinSF2", "TotalBsmtSF", "HeatingQC", 
    "CentralAir", "X1stFlrSF", "X2ndFlrSF", "GrLivArea", "FullBath", "HalfBath", 
    "BedroomAbvGr", "KitchenQual", "TotRmsAbvGrd", "GarageType", "GarageYrBlt", 
    "GarageCars", "GarageArea", "GarageQual", "GarageCond", "OpenPorchSF", 
    "EnclosedPorch", "ScreenPorch", "Fence", "MiscFeature", "MiscVal", "MoSold", 
    "YrSold", "SalePrice")
...
rownames(first_half_corHeap) <- c("0", rownames(corHeap)[1:length_corHeap])

second_half_corHeap <- as.data.frame(corHeap[(length_corHeap + 1):NROW(corHeap), ])
...
rownames(second_half_corHeap) <- c(
    "0", rownames(corHeap)[(length_corHeap + 1):NROW(corHeap)])
...
HP$AlleyExists <- factor(!is.na(Alley), levels = c(F, T), labels = c("no", "yes"))
HP$LotShapeLess <- factor(LotShape == "Reg", levels = c(T, F), labels = c("Regular", "Irregular"))
HP$HouseAge <- current_year - YearBuilt
HP$TotalBaths <- FullBath + HalfBath
...
HP$FireplacesExists <- factor(Fireplaces != 0, levels = c(F, T), labels = c("no", "yes"))
HP$FenceExists <- factor(!is.na(Fence), levels = c(F, T), labels = c("no", "yes"))
...
```{r, warning = F}
vars <- c(
    "FrontageExists", "AlleyExists", "LotShapeLess", "HouseAge", "TotalBaths",
    "GarageExists", "OpenPorchExists", "EnclosedPorchExists", "ScreenPorchExists", 
    "FireplacesExists", "FenceExists")

corHeapCalc <- data.frame(
    SalePrice = rep(0, length(vars)))
rownames(corHeapCalc) <- vars


for (v in vars){
    corHeapCalc[v, "SalePrice"] = cor(as.numeric(HP[, v]), SalePrice)
}

ggcorrplot(
    rbind.data.frame(data.frame(SalePrice = 0), corHeapCalc), 
    type = "lower", 
    lab = TRUE, 
    lab_size = 2.5) + 
    theme(
...
totalCorHeap = rbind.data.frame(corHeap, corHeapCalc)
