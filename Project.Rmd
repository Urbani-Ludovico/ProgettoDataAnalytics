...
for (package in c("ggplot2", "ggcorrplot", "ggpubr", "ggfortify", "dplyr", "tidyr", 
                  ...
                  "cluster", "factoextra", "dbscan", "dendextend")) {
...
## Analisi della qualità del riscaldamento con l'anno degli ultimi lavori
...
```{r, message = F, warning = F}
for (col in names(HP_normal)) {
    if (is.numeric(HP_normal[[col]])) {
        HP_normal[[col]][is.na(HP_normal[[col]])] <- mean(HP_normal[[col]], na.rm = TRUE)
    } else {
        HP_normal[[col]][is.na(HP_normal[[col]])] <- as.character(sort(table(HP_normal[[col]])[1]))
    }
}
```
...
```{r, message = F, warning = F}
HP_normal$cluster <- kmeans_result$cluster
...
info_text <- paste(
    ...
    paste("Cluster 1:", length(which(sil[, 1] == 1)), "|", 
          round(summary_sil$clus.avg.widths[1], 2)), "\n",
    paste("Cluster 2:", length(which(sil[, 1] == 2)), "|", 
          round(summary_sil$clus.avg.widths[2], 2)), "\n",
    paste("Cluster 3:", length(which(sil[, 1] == 3)), "|", 
          round(summary_sil$clus.avg.widths[3], 2)), "\n",
    paste("Cluster 4:", length(which(sil[, 1] == 3)), "|", 
          round(summary_sil$clus.avg.widths[4], 2)))
...
```{r, message = F, warning = F}
ggplot(HP_normal, aes(x = factor(cluster), y = SalePrice, fill = factor(cluster))) +
    geom_boxplot() +
    labs(title = "Distribuzione di SalePrice tra i Cluster", 
         x = "Cluster", y = "SalePrice")
```
...
sil <- silhouette(kmeans_result$cluster, dist(scaled_data))
p <- fviz_silhouette(sil) +
    labs(title = "Silhouette Plot per K-means Clustering con Variabili Estese", 
         subtitle = "3 Clusters")
...
selected_data_extended <- HP_normal %>%
    dplyr::select(SalePrice, OverallQual, GrLivArea, all_of(additional_vars))
...
fviz_cluster(kmeans_result_extended, data = scaled_data_extended, 
             ellipse.type = "convex") +
...
```{r, message = F, warning = F}
HP_normal$cluster_extended <- kmeans_result_extended$cluster

# Calcolo della media di ciascuna variabile selezionata per ciascun cluster
centroids_extended <- HP_normal %>%
...
```{r, message = F, warning = F}
print(table(HP_normal$cluster_extended))
```
...
HP_normal$cluster_agnes1 <- cut_agnes1

centroids_agnes1 <- HP_normal %>%
    group_by(cluster_agnes1) %>%
    summarise(across(where(is.numeric), mean, na.rm = TRUE))

print(centroids_agnes1, width = Inf)
print(table(HP_normal$cluster_agnes1))
```
...
HP_normal$cluster_agnes2 <- cut_agnes2

centroids_agnes2 <- HP_normal %>%
    group_by(cluster_agnes2) %>%
    summarise(across(where(is.numeric), mean, na.rm = TRUE))

print(centroids_agnes2, width = Inf)
print(table(HP_normal$cluster_agnes2))
```
...
fviz_cluster(dbscan_result1, data = scaled_data, geom = "point") +
    labs(title = "DBSCAN Clustering (Top 2 Variables)", 
         subtitle = "eps = 0.5, minPts = 10")

HP_normal$cluster_dbscan1 <- dbscan_result1$cluster

dbscan_centroids1 <- HP_normal %>%
    group_by(cluster_dbscan1) %>%
    summarise(across(where(is.numeric), mean, na.rm = TRUE))

print(dbscan_centroids1, width = Inf)
print(table(HP_normal$cluster_dbscan1))
```
...
fviz_cluster(dbscan_result2, data = scaled_data_extended, geom = "point") +
    labs(title = "DBSCAN Clustering (Extended Variables)", 
         subtitle = "eps = 0.5, minPts = 10")

HP_normal$cluster_dbscan2 <- dbscan_result2$cluster

dbscan_centroids2 <- HP_normal %>%
    group_by(cluster_dbscan2) %>%
    summarise(across(where(is.numeric), mean, na.rm = TRUE))

print(dbscan_centroids2, width = Inf)
print(table(HP_normal$cluster_dbscan2))
```
